...
public class Main {
    ...
    private static final String OUTFORMAT = "-outformat";   // Optional: xml/html/json
        ...
        if (args.length > 0) {
            for (int i = 0; i < args.length; i++) {
                ...
                if (args[i].equals(FILEPATH)) {
                    ...
                    outFormat = args[i + 1].toUpperCase();
            ...
            if(!token.equals("")){
                if (!method.equals("") && !url.equals("") && !filePath.equals("")) {
                    ...
                    RequestOperations.makeStoreRequestCall(lstRequestObject, outFormat, outFile, token);
                } else if (!filePath.equals("")) {
                    RequestsList lstRequests = JSONReader.getRequestsList(filePath);
                    RequestOperations.makeStoreRequestCall(lstRequests, outFormat, outFile, token);
                }
            }else {
    ...
    private static void showHelp() {
        ...
        System.out.println(OUTFORMAT + "\t<FORMAT> \t \t Preferred format the client will return (XML, HTML, JSONdefault format JSON) (Optional)");
